# Scala SBT плагин для интеграции с NPM. (Сборка ES6 проекта в единый ear архив).

У нас в Scala проекте использовался свой ear-паковщик, написанный на SBT, поэтому пришлось написать свой плагин 
для сборки ES6 проекта и интеграции его в текущий механизм сборки.

* При сборке проекта плагин запускает NPM процесс в системе. 
* Устанавливает все необходимые зависимости для ES6 проекта в рабочую директорию.
* Запускает команду build, которая получает сгенерированный JSON файл конфигурации для клиента. 
* Далее запускает Webpack. Webpack собирает ES6 проект. 
* Сгенерированные файлы помещаются в war архив. 
* War архив помещается в единый ear архив который развертывается в J2EE контейнере.
